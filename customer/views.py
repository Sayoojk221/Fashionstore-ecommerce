...
def outofstock():
    ...
    if value:
                                                # first filtering zero coloumn in size table.if have any zero
        totalid = [i for j in value for i in j]
                                                # row exist , get productid of that row. then check total count of each productid
        out_of_stock = []
...
def emailpasswordlogin(request):
    ...
    if password_deatils:
        # here converting database value to list format
    ...
    if request.method == 'POST':
        ...
        if not check:
            ...
            register.objects.create(
                email=email, phoneno=phone, password=hashpassword)
            return redirect(url)
        else:
            return redirect(url)
    else:
...
def product_single(request):
    ...
    value2 = ProductSize.objects.filter(productcolor__colorid=colorid).values_list('productcolor__productcommon__category')
    category = [i for j in value2 for i in j ]
    productcategory = ProductLists.objects.filter(productfulldetails__productcolor__productcommon__category=category[0]).order_by('-productfulldetails__productcolor__productcommon__createddate')
    stock = outofstock()
...
def generate_order_id(price,customerid):
    generate = ''.join(["{}".format(random.randint(0, 9)) for num in range(0, 12)])
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        address = f'{fname}.{lname},{house},{city}-{pincode},{state} {nearlocation} {phone}'
...
def category(request):
    ...
    value = ProductSize.objects.all().values_list('productcolor')
    colorsize = [i for j in value for i in j]
    context = {'color':productcolor,'colorsize':colorsize,'stock':stock,'products':products,'customerexist':customer,'procount': productcount, 'totalprice': totalprice,'cart':cartlist}
    return render(request,'customer/homepage/category.html',context)


def filter(request,cat=None,gen=None,siz=None,bra=None,pri=None):
    query = ProductLists.objects.all()
    stock = outofstock()
    productcolor = ProductColor.objects.all()
    value = ProductSize.objects.all().values_list('productcolor')
    colorsize = [i for j in value for i in j]
    ...
    if cat != 'all':
        value = cat.split('&')
        category = [i for i in value if i]
        query = query.filter(productfulldetails__productcolor__productcommon__category=category[0])


    if gen != 'all':
        value = gen.split('&')
        gender = [x for x in value if x]
        query = query.filter(productfulldetails__productcolor__productcommon__gender=gender[0])


    if siz != 'all':
        value = siz.split('&')
        size = [i for i in value if i]
        query = query.filter(productfulldetails__size__in=size)


    if bra != 'all':
        value = bra.split('&')
        brand = [i for i in value if i]
        query = query.filter(productfulldetails__productcolor__productcommon__brand__in=brand)

    if pri:
        value = pri.split(',')
        item = [i for i in value if i]
        query = query.filter(Q(productfulldetails__price__gt=int(item[0]))&Q(productfulldetails__price__lte=int(item[1])))

    context = {'color':productcolor,'colorsize':colorsize,'stock':stock,'products':query,'customerexist':customer,'procount': productcount, 'totalprice': totalprice,'cart':cartlist}
    return render(request,'customer/homepage/category.html',context)
